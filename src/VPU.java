...
//    inicializa un nuevo proceso en la cola del arreglo de procesos
    public boolean nuevoProceso(String name, int time, int priori){
        ...
        while (!writted && index <= this.maxLengt) {            
            ...
            if (this.arProcesos[index] == null && !writted) {
                ...
                this.arProcesos[index] = new Proceso(name, time, priori);
                writted = !writted;
            }
            index++;
        }
...
//   actualiza los distintos campos de la vista en tiempo de ejecucion
    public void muestra() {
        ...
        table = new DefaultTableModel(new Object[]{"Proceso", "Quantums", "Priority"}, 0);
        
        for (Proceso arProceso : this.arProcesos) {
            if (arProceso != null) {
                table.addRow(new Object[] {arProceso.getName(), arProceso.getQuantums(), arProceso.getPrioridad()});
